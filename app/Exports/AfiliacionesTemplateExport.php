...
class AfiliacionesTemplateExport implements WithHeadings, WithStyles, WithTitle
{
    public function headings(): array
    {
        return [
            ...
            [
                ...
                'Número de meses del contrato. Ej: 6 (dejar vacío si no aplica)',
                'Número de días del contrato. Ej: 15 (dejar vacío si no aplica)',
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getColumnDimension('F')->setWidth(22);  // FECHA DE NACIMIENTO *
        $sheet->getColumnDimension('G')->setWidth(18);  // No. CELULAR *
        $sheet->getColumnDimension('H')->setWidth(22);  // VALOR DEL CONTRATO *
        $sheet->getColumnDimension('I')->setWidth(22);  // HONORARIOS MENSUAL *
        $sheet->getColumnDimension('J')->setWidth(22);  // IBC *
        $sheet->getColumnDimension('K')->setWidth(12);  // MESES
        $sheet->getColumnDimension('L')->setWidth(12);  // DIAS
        $sheet->getColumnDimension('M')->setWidth(28);  // FECHA INGRESO *
        $sheet->getColumnDimension('N')->setWidth(22);  // FECHA RETIRO *
        $sheet->getColumnDimension('O')->setWidth(35);  // SECRETARÍA *
        $sheet->getColumnDimension('P')->setWidth(30);  // ÁREA
        $sheet->getColumnDimension('Q')->setWidth(20);  // NIVEL DE RIESGO *
        $sheet->getColumnDimension('R')->setWidth(22);  // NOMBRE ARL *
        $sheet->getColumnDimension('S')->setWidth(25);  // BARRIO
        $sheet->getColumnDimension('T')->setWidth(35);  // DIRECCIÓN RESIDENCIA
        $sheet->getColumnDimension('U')->setWidth(25);  // EPS
        $sheet->getColumnDimension('V')->setWidth(25);  // AFP
        $sheet->getColumnDimension('W')->setWidth(30);  // CORREO ELECTRÓNICO
        $sheet->getColumnDimension('X')->setWidth(28);  // FECHA DE AFILIACIÓN ARL
        $sheet->getColumnDimension('Y')->setWidth(32);  // FECHA TERMINACIÓN AFILIACIÓN ARL
